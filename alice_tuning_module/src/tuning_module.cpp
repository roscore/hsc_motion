...
void TuningModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
       ...
       it != robot->dxls_.end(); it++)
  {
    ...
    temp_offset_data_[joint_name] = 0;
  ...
  base_path_ = ros::package::getPath("alice_base_module") + "/data/ini_pose_"+alice_id_+".yaml";
  offset_path_=""; 
  previous_offset_mode_=true;
  previous_offset_pose_=false;
  ros_node.param<std::string>("offset_file_path", offset_path_, offset_path_);
 
  parseOffsetData_temp(offset_path_,"offset");
}
void TuningModule::readIDData()
{
  ros::NodeHandle nh;
...
void TuningModule::moveToTunePose(const std::string &pose_name)
{
  if(enable_ == false)
  {
    ROS_ERROR("alice_tuning_module is not enable!!");
    return;
  }

  //get target pose from yaml file
  
  if(pose_name =="zero_offset_pose")
  {     
    parseOffsetData(base_path_,"zero",0);
    parseTunePoseData(offset_path_, "offset",-1);

  }
  else if(pose_name =="pre_offset_pose")
  {    
    parseOffsetData(base_path_,"zero",0);
    parseTunePoseData(offset_path_, "offset",0);
  }
...
bool TuningModule::parseOffsetData_temp(const std::string &path, const std::string &pose)
{
  ...
  YAML::Node offset_data_node = doc[pose];

  if(offset_data_node == NULL)
  {
  ...
  for (YAML::const_iterator node_it = offset_data_node.begin(); node_it != offset_data_node.end(); node_it++)
  {
    ...
    std::map<std::string, double>::iterator temp_offset_data_it = temp_offset_data_.find(joint_name);
    if (temp_offset_data_it != temp_offset_data_.end())
    {
      temp_offset_data_[joint_name] = offset;
...
bool TuningModule::parseOffsetData(const std::string &path, const std::string &pose, int dir)
{
  ...
  if (pose == "zero")
  {
    tar_pose_node = offset_data_node["target_pose"];
  }
  else if (pose == "offset") 
  {
    tar_pose_node = offset_data_node;
  }


  for (YAML::const_iterator node_it = tar_pose_node.begin(); node_it != tar_pose_node.end(); node_it++)
  {
    std::string joint_name = node_it->first.as<std::string>();
    double offset = node_it->second.as<double>();
    ...
    {
      robot_tuning_data_[joint_name]->joint_offset_rad_ = dir*offset;
    }
  }
...
bool TuningModule::parseTunePoseData(const std::string &path, const std::string &pose, int dir)
{
  ...
  ROS_INFO("%s",pose.c_str());
    tar_pose_node = init_pose_node;    
  }
  else
  { 
    ...
    tar_pose_node = init_pose_node["target_pose"];    
  }
  ...
  for (YAML::iterator yaml_it = tar_pose_node.begin(); yaml_it != tar_pose_node.end(); ++yaml_it)
  {
    ...
    tuning_module_state_->joint_pose_.coeffRef(id, 0) = dir * value;

    std::map<std::string, JointOffsetData*>::iterator robot_tuning_data_it = robot_tuning_data_.find(joint_name);
    if (robot_tuning_data_it != robot_tuning_data_.end())
      robot_tuning_data_[joint_name]->goal_position_ = dir * value;
  }
...
void TuningModule::queueThread()
{
  ...
  present_joint_pub_ = ros_node.advertise<alice_tuning_module_msgs::JointsOffsetPositionData>("/robotis/tuning_module/present_joints_data", 1);
...
void TuningModule::tunePoseMsgCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  if (msg->data == "zero_offset_pose")
  {
    previous_offset_pose_ = true;
    moveToTunePose(msg->data);
  }
  else if (msg->data == "pre_offset_pose")
  {
    previous_offset_pose_ = true;
    moveToTunePose(msg->data);
  }
...
void TuningModule::commandCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  else if (msg->data == "load_offset")
  {
    // send a command to robotis_controller to load the new offset file
    loadOffsetToController(offset_path_);
  }
  else if (msg->data == "offset_on")
  {
    if(previous_offset_mode_==true)
    {
      ROS_INFO("Already offset on");
      return;
    }
    turnOnOffOffset(true);
    previous_offset_mode_=true;
    parseOffsetData(offset_path_,"offset",1);
  }

  else if (msg->data == "offset_off")
  {

    if(previous_offset_mode_==false)
    {
      ROS_INFO("Already offset off");
      return;
    }
    turnOnOffOffset(false);
    previous_offset_mode_=false;
    parseOffsetData(base_path_,"zero",1);
...
void TuningModule::jointOffsetDataCallback(const alice_tuning_module_msgs::JointOffsetData::ConstPtr &msg)
{
  ...
  //goal position
  ROS_INFO_STREAM(
        msg->joint_name << " " << msg->goal_value << " " << msg->offset_value );
...
void TuningModule::jointTorqueOnOffCallback(const alice_tuning_module_msgs::JointTorqueOnOffArray::ConstPtr& msg)
{  
...
bool TuningModule::getPresentJointOffsetDataServiceCallback(
    alice_tuning_module_msgs::GetPresentJointOffsetData::Request &req,
    alice_tuning_module_msgs::GetPresentJointOffsetData::Response &res)
{
       ...
       map_it != robot_tuning_data_.end(); map_it++)
  {
    ...
    alice_tuning_module_msgs::JointOffsetPositionData joint_offset_pos;
...
{
  alice_tuning_module_msgs::JointsOffsetPositionData present_data;
...
void TuningModule::saveOffsetToYaml(const std::string &path)
{
       ...
       map_it != robot_tuning_data_.end(); map_it++)
  {
    ...
    if(previous_offset_pose_ == true)
    {
      offset[joint_name] = joint_data->joint_offset_rad_ + joint_data->goal_position_ + temp_offset_data_[joint_name];
    }
    else
    {
