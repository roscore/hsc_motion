...
void ActionModule::readIDData()
{
  ros::NodeHandle nh;
  int alice_id_int  = nh.param<int>("alice_userid",0);

  ROS_INFO("Action id: %d",alice_id_int);
...
void ActionModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  std::string path = ros::package::getPath("alice_action_module") + "/data/alice_motion_4095_"+alice_id_+".bin";
...
bool ActionModule::isRunningServiceCallback(alice_action_module_msgs::IsRunning::Request &req,
                                            alice_action_module_msgs::IsRunning::Response &res)
{
...
void ActionModule::startActionCallback(const alice_action_module_msgs::StartAction::ConstPtr& msg)
{
  if (enable_ == false)
  {
    std::string status_msg = "Action Module is not enabled";
...
void ActionModule::onModuleEnalice()
{
  action_module_enabled_ = true;
}

void ActionModule::onModuleDisalice()
{
  action_module_enabled_ = false;
  brake();
}
...
bool ActionModule::start(int page_number, action_file_define::Page* page)
{
  if (enable_ == false)
  {
    std::string status_msg = "Action Module is disaliced";
    ROS_ERROR_STREAM(status_msg);
    publishStatusMsg(robotis_controller_msgs::StatusMsg::STATUS_ERROR, status_msg);
    return false;
  }
